...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isDirectory()) {
            file = new File(root, java.net.URLDecoder.decode(request.getUri() + "/index.html", "UTF-8"));
        }
        if (file.isFile()) {
            byte[] body = readFile(file);
            String eTagHash = Integer.toString(Arrays.hashCode(body));
            if (request.getHeader("If-None-Match") != null && request.getHeader("If-None-Match").equals(eTagHash)) {
            ...
            response.setHeader("ETag", eTagHash);
        } else {
