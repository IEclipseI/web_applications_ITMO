...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        try {
            ...
            for (Class<?> o :
                    parameterTypes) {
                if (o.equals(Map.class)) {
                    list.add(view);
                } else if (o.equals(HttpServletRequest.class)) {
                    list.add(request);
                }
            }
            ((BasePage)page).before(request, view);
            method.invoke(page, list.toArray());
            ((BasePage)page).after(request, view);
